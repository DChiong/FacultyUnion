...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['add_award'])) {
    ...
    if (move_uploaded_file($_FILES["award_image"]["tmp_name"], $upload_file)) {
        ...
        $other_workspace_dir = $_SERVER['DOCUMENT_ROOT'] . "/faculty-union-main/uploads/awards/";
        if (is_dir(dirname(dirname($other_workspace_dir)))) {
            if (!is_dir($other_workspace_dir)) mkdir($other_workspace_dir, 0777, true);
            copy($upload_file, $other_workspace_dir . $file_name);
        }
...
<div class="container content mt-2 pb-5">
    ...
    <div class="d-flex justify-content-between align-items-center mb-4">
        <div>
            <button class="btn btn-maroon" data-bs-toggle="modal" data-bs-target="#addAwardModal">
                <i class="bi bi-plus-lg me-1"></i> Add Award
            </button>
        </div>
        <div class="text-muted small">Total awards: <?php echo count($awards); ?></div>
    </div>
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td class="text-center">
                            <button type="button" class="btn btn-sm btn-outline-info" onclick="openEditModal(<?php echo htmlspecialchars(json_encode($row)); ?>)">
                                <i class="fas fa-edit"></i> Edit
                            </button>
                            <a href="?delete=<?php echo $row['id']; ?>" class="btn btn-sm btn-outline-danger" onclick="return confirm('Delete this record?')">
...
<div class="modal fade" id="addAwardModal" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            <form action="" method="POST" enctype="multipart/form-data">
                <div class="modal-header btn-maroon text-white">
                    <h5 class="modal-title">Add New Faculty Award</h5>
                    <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <div class="modal-body">
                    <div class="row g-3">
                ...
                <div class="modal-footer">
                    <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Close</button>
                    <button type="submit" name="add_award" class="btn btn-maroon">Publish Award</button>
                </div>
            </form>
        </div>
    </div>
